...
//            return '请用支付宝或者微信扫描二维码';
    ...
    public function autoStorePost(Request $request)
    {
        ...
        if(!$store['business_license_img_url']){
            array_forget($store,'business_license_img_url');
        }
        $aop = $this->AopClient();
        $aop->method = "fshows.liquidation.submerchant.create.with.auth";
        $data = array('content' => json_encode($store));
   ...
   public function witnessAutomPost(Request $request)
    {
        ...
        $mshop=MerchantShops::where('store_id',$data['external_id'])->where('store_type','pingan')->first();
        if($mshop){
            $m_id=$mshop->merchant_id;
        }else{
            $m_id=0;
        }
        $witnessaccount=PinganWitnessAccount::where('merchant_id',$m_id)->first();
