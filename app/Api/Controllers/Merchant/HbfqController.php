...
class HbfqController extends BaseController
{

    public function AlipayHbfq(Request $request)
    {

        try {
            $user = $this->getMerchantInfo();
            $shop_id = $request->get('shop_id');//口碑门店id
            $total_amount = $request->get('total_amount');//金额
            $hb_fq_num = $request->get('hb_fq_num', 3);//花呗分期数
            $hb_fq_seller_percent = $request->get('hb_fq_seller_percent', 0);//商家承担手续费传入100，用户承担手续费传入0
