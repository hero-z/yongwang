...
\App\Common\Log::write($rsp,'saomaqiang_yinlian_bank_return.log.txt');
...
// file_put_contents('./yinlian_verify.txt', var_export($verify,true),FILE_APPEND);
 ...
 /////////////////end///////////// 开始收款
    ...
    */
    ...
    { 
\App\Common\Log::write($request->all(),'saomaqiang_in.log.txt');
        $user = $this->getMerchantInfo();
...
//                        $liquidator_commission_fee=$data['total_amount']*$data['liquidator_commission_rate'];
        ...
        /****************银联 开始*******************/
        if ($str == '62') {
\App\Common\Log::write('进入银联通道','saomaqiang_yinlian_in.log.txt');
            set_time_limit(0);
            $union_return=$this->union($no,$m_id,$code,$store->store_id,$total_amount);
            $return_data=[];
            if($union_return['status']==1)
            {
                $order=Order::where('out_trade_no',$no)->first();
                if(!empty($order))
                $return_data['trade_no']=$order->trade_no;
                else
                $return_data['trade_no']=$no;
            ...
            $return_data['msg']=$union_return['message'];
            $return_data['out_trade_no']=$no;

\App\Common\Log::write($return_data,'saomaqiang_yinlian_return.log.txt');
