...
@section('content')
    ...
    <input type="hidden" id="store_id" value="{{$shop->external_id}}">
    ...
    <div class="keyboard">
        <ul>
            ...
            {{--<li class="confirm" data-txt="确认" >--}}
                {{--<button onclick="callpay()"  id="payLogButton"  style="width: 100%;height: 100%;background-color: transparent;border: none;outline: medium"></button>--}}
            {{--</li>--}}
            <li data-num="4"></li>
            <li data-num="5"></li>
            <li data-num="6"></li>
        ...
        function callpay() {
            if(ck&&$("#price").html()){
                ...
                $.post("{{route('PAWxOrder')}}", {
                        external_id: $("#store_id").val(),
                        _token: "{{csrf_token()}}",
                        total_fee: $("#price").html(),
                        remark:$("#remark").val(),
                        m_id: $("#m_id").val()
                    },
                    function (data) {
                        ck=1;
                        if (data.success) {
                            window.location.href = 'https://openapi-liquidation.51fubei.com/payPage/?prepay_id=' + data.return_value.prepay_id + '&callback_url=' + '{{url('/admin/pingan/weixin/orderStatus')}}';
                        } else {
