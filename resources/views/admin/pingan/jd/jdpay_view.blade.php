    ...
    <script>
        function callpay() {
            $.post("{{route('jdpost')}}", {
                ...
                if (response.success){
                    window.location.href ="https://openapi-liquidation.51fubei.com/jdPayPage?prepay_id="+ response.return_value.prepay_id+"&trade_no="+response.return_value.trade_no+"&body="+data.body+"门店收款信息"+"&callback_url={{route('jd_url')}}";
                }else {
