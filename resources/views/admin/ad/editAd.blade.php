                    ...
                    <div class="col-sm-12">
                        <form action="{{route('insertAd')}}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control m-b"  id="type" name="type" required="required">
                                        <option value="alipay" @if($list->type=="alipay") selected="selected" @endif>支付宝</option>
                                        <option value="weixin" @if($list->type=="weixin") selected="selected" @endif>微信</option>
                                        <option value="jd" @if($list->type=="jd") selected="selected" @endif>京东钱包</option>
                                        <option value="bestpay" @if($list->type=="bestpay") selected="selected" @endif>翼支付</option>
                                        <option value="pufa" @if($list->type=="pufa") selected="selected" @endif>浦发银行</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control m-b"  id="position" name="position" required="required">
                                        <option value="1" @if($list->position==1) selected="selected" @endif>支付成功页</option>
                                        <option value="0" @if($list->position==0) selected="selected" @endif>支付失败页</option>
                                    </select>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control m-b" id="status" name="status" required="required">
                                        <option value="1" @if($list->status==1) selected="selected" @endif>启用</option>
                                        <option value="0" @if($list->status==0) selected="selected" @endif>下线</option>
                                    </select>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            ...
                            <div class="form-group">
                                <label>开始时间</label>
                                <div class="col-sm-10">
                                    <input size="16" type="text" id="time_start" name="time_start" value="@if(!empty($list->time_start)){{$list->time_start}}@else{{date('Y-m-d').' 00:00'}}@endif"  class="form-datetime-lang am-form-field" required="required">
                            ...
                            <div class="form-group">
                                <label>结束时间</label>
                                <div class="col-sm-10">
                                    <input size="16" type="text" id="time_end" name="time_end" value="@if(!empty($list->time_end)){{$list->time_end}}@else{{date('Y-m-d').' 23:59'}}@endif"  class="form-datetime-lang am-form-field" required="required">
                            ...
                            <div class="form-group">
                                ...
                                <input msg="广告描述" placeholder="给广告一个描述,方便自己查看" class="form-control"
                                       name="content" id="content" type="text" value="{{$list->content}}">
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <input type="hidden" required="required" size="50" value="{{$list->pic}}" name="oldpic" id="oldpic">
                                <!-- 图片上传按钮 -->
                                <input id="fileupload" type="file" name="image" data-url="{{route('uploads')}}"
                                       data-form-data='{"_token": "{{csrf_token()}}"}' multiple="true" >
                                <!-- 图片展示模块 -->
                                <div class="files" ><img class="images_zone" id="oldimg" width="30px" src="{{url($list->pic)}}"></div>
