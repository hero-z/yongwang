...
<div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
    <div class="widget am-cf">
        <form method='post' class="am-form tpl-form-line-form" action="{{route('statistics.billquery')}}">
            ...
            <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                <select data-am-selected="{btnSize: 'sm',searchBox: 1,maxHeight: 200}" id="admin_id" name="admin_id" style="display: none;" onchange="change()">
                    <option value="0" >员工归属</option>
                    @if($users)
                        @foreach($users as $k=>$v)
                            <option value="{{$k}}" @if($admin_id&&$admin_id==$k) selected @endif>{{$v}}</option>
                        @endforeach
                    @endif
                </select>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="dataTables_paginate paging_simple_numbers"
                        ...
                        @if($list)
                            {{$list->appends(compact('admin_id','paytype','paystatus','time','time_start','time_end'))->render()}}
                        @endif
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    function exportdata() {
        console.log("{{route('statistics.billquery')}}")
        window.location.href="{{route('statistics.billquery')}}"+"?export=1&admin_id="+$('#admin_id').val()+"&paytype="+$('#paytype').val()+"&time="+$('#time').val()+"&paystatus="+$('#paystatus').val()+"&time_end="+$('#time_end').val()+"&time_start="+$('#time_start').val();
...
<script>
    $(function(){
        $.post("{{route('statistics.billquery')}}", {
                _token: "{{csrf_token()}}",
