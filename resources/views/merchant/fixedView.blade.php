...
<div class="weui-wepay-pay-select">
    ...
    <div class="weui-wepay-pay-select__ft">
        <div class="weui-wepay-pay__btn">
            <a href="javascript:;" class="weui-btn weui-btn_primary" onclick="fun('{{$type}}')">立即生成付款码支付</a>
        </div>
    </div>
    <div class="js_dialog" id="iosDialog3" style="opacity: 1;display:none">
        <div class="weui-mask"></div>
        <div class="weui-dialog">
            <div class="weui-dialog__bd" id="msg"></div>
            <div class="weui-dialog__ft">
    ...
    <div class="weui-wepay-logos weui-wepay-logos_ft">
        ...
        @endif
        @if($type=="o")
            <img style="vertical-align:middle" src="{{url('/img/alipay_logo.png')}}" alt="" height="32">
        @endif
        @if($type=="w")
            <img style="vertical-align:middle"
                 src="https://act.weixin.qq.com/static/cdn/img/wepayui/0.1.1/wepay_logo_green_200x200.png" alt=""
                 height="32">
        @endif
        @if($type=="pj")
            <img style="vertical-align:middle" src="{{url('/img/jd_logo.png')}}" alt="" height="32">
        @endif
        @if($type=="f")
            <img style="vertical-align:middle" src="{{url('/img/alipay_logo.png')}}" alt="" height="32">
        @endif
    ...
    <script>
        function fun(type) {
            if (type == "u") {
                ...
                $url = "{{route('getcodeurl')}}"
            }
            if (!$("#total_amount").val()) {
                alert('请输入金额');
                $('#totla_amount').onfocus;
            } else {
                $.post($url, {
                        _token: "{{csrf_token()}}",
                        store_id: "{{$store_id}}",
