...
<div class="weui-wepay-pay-select">
    ...
    padding-right: 14px;
    text-align: center;
    text-decoration: none;">选择支付方式</label>

    @foreach($store_id as $k=>$v)
        @if($k=='u')
            <a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k,'m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">
                <img  style="vertical-align:middle" src="{{url('/img/upay_logo.png')}}" alt="" height="32">银联
            </a>
            @continue($k=='u')
        @elseif($k=='w')
            <a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k,'m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">
                <img  style="vertical-align:middle"  src="https://act.weixin.qq.com/static/cdn/img/wepayui/0.1.1/wepay_logo_green_200x200.png" alt="" height="32">官方微信
            </a>
            @continue($k=='w')
        @elseif($k=='o')
            <a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k,'m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">
                <img  style="vertical-align:middle" src="{{url('/img/alipay_logo.png')}}" alt="" height="32">官方支付宝当面付
            </a>
            @continue($k=='o')
        @elseif($k=='s')
            <a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k,'m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">
                <img  style="vertical-align:middle" src="{{url('/img/alipay_logo.png')}}" alt="" height="32">官方支付宝口碑
            </a>
            @continue($k=='s')
        @elseif($k=='p')
            <a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k.'j','m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">
                <img  style="vertical-align:middle" src="{{url('/img/jd_logo.png')}}" alt="" height="32">平安京东
            </a>
            @continue($k=='p')
        {{--@elseif($k=='f')--}}
            {{--<a href="{{route('allPayFixed',['store_id'=>$v,'type'=>$k,'m_id'=>$m_id])}}" class="weui-btn weui-btn_plain-default">--}}
                {{--<img  style="vertical-align:middle" src="{{url('/img/alipay_logo.png')}}" alt="" height="32">浦发银行--}}
            {{--</a>--}}
            {{--@continue($k=='f')--}}
        @endif
    @endforeach
